...
class PredatorBird(Bird):
    ...
    def run_network(self):
        actions = self.network.activate(self.sight_sensors + [self.energy - 30])
    ...
    def draw(self):
        ...
        pygame.gfxdraw.aacircle(self.env.win, int(self.x), int(self.y), 8, (
            150, min(150 - int(150 * self.energy / 100.0), 255),
            min(150 - int(149 * self.energy / 100.0), 255)))
        pygame.gfxdraw.aacircle(self.env.win, int(self.x), int(self.y), 7, (
            150, min(150 - int(150 * self.energy / 100.0), 255),
            min(150 - int(149 * self.energy / 100.0), 255)))
        pygame.gfxdraw.aacircle(self.env.win, int(self.x), int(self.y), 6, (
            150, min(150 - int(150 * self.energy / 100.0), 255),
            min(150 - int(149 * self.energy / 100.0), 255)))

        pygame.draw.aaline(self.env.win, DARK_RED, [int(self.x), int(self.y)],
                           [int(self.x + 12 * math.cos(a)),
                            int(self.y + 12 * math.sin(a))])
